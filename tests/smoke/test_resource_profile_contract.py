...
class BackendResourceProfileContract(unittest.TestCase):
    def test_migration_adds_column_with_check(self) -> None:
        sql = (APP / "migrations" / "0005_project_resource_profile.sql").read_text(encoding="utf-8")
    ...
    def test_custom_profile_is_allowed_by_migration_0006(self) -> None:
        sql = (APP / "migrations" / "0006_resource_profile_custom.sql").read_text(encoding="utf-8")
    ...
    def test_schemas_accept_only_known_profiles(self) -> None:
        source = (APP / "schemas.py").read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_settings_whitelist_and_derived_guard(self) -> None:
        source = (APP / "project_settings.py").read_text(encoding="utf-8")
    ...
    def test_worker_reads_the_persisted_profile(self) -> None:
        ...
        backgrounds = (APP / "project_backgrounds.py").read_text(encoding="utf-8")
        worker = backgrounds.split("async def _provision_and_store_keys", 1)[1]
        worker = worker.split("\nasync def ", 1)[0]
    ...
    def test_profile_is_persisted_next_to_the_limits_it_derives(self) -> None:
        ...
        settings = (APP / "project_settings.py").read_text(encoding="utf-8")
    ...
    def test_migration_keeps_the_project_own_profile(self) -> None:
        ...
        tool = (ROOT / "tools/migrate_project_resource_limits.py").read_text(encoding="utf-8")
    ...
    def test_scripts_forward_override_to_helper(self) -> None:
        for name in (
            ...
            with self.subTest(script=name):
                self.assertIn(
                    "${PROJECT_RESOURCE_PROFILE_OVERRIDE:-}", path.read_text(encoding="utf-8")
                )
