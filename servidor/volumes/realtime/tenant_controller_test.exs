defmodule RealtimeWeb.TenantControllerTest do
  ...
  describe "delete tenant" do
    ...
    test "deletes chosen tenant", %{conn: conn, tenant: tenant} do
      ...
      %{rows: [rows]} = Postgrex.query!(db_conn, "SELECT slot_name FROM pg_replication_slots", [])
      ...
      assert {:ok, %{rows: []}} = Postgrex.query(db_conn, "SELECT slot_name FROM pg_replication_slots", [])
    end
