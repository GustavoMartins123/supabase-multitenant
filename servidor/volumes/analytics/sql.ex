  ...
  defp do_replace_sandboxed_query(
    ...
    case extract_replacement_query_from_ast(ast, data) do
      {:ok, %{"with" => with_clause} = replacement_query} when not is_nil(with_clause) ->
        # The sandboxed (caller-supplied) query already has its own WITH clause.
        # Nesting it as `(WITH ... SELECT ...)` inside the template's own WITH
        # produces invalid Postgres ("multiple WITH/LIMIT clauses not allowed"),
        # so merge its CTEs into the template's CTE list and let its own
        # body/order_by/limit become the outer statement's.
        merged_cte_tables =
          get_in(sandbox_query, ["with", "cte_tables"]) ++
            Map.get(with_clause, "cte_tables", [])

        merged_with = Map.put(with_clause, "cte_tables", merged_cte_tables)

        {k,
         sandbox_query
         |> Map.merge(%{
           "with" => merged_with,
           "body" => replacement_query["body"],
           "order_by" => replacement_query["order_by"],
           "limit" => replacement_query["limit"]
         })}
