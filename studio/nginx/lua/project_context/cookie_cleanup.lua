...
local function expired_cookie_header()
    return "supabase_project=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT"
end
...
if not expect or string.lower(expect) ~= string.lower(sig) then
    return expired_cookie_header()
end
